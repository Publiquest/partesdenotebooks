...
////////////////////////////////////////////////////////////////////
// Get Tags from Post by Category 
////////////////////////////////////////////////////////////////////
function get_tags_from_category_post($id)
    {
        $args=array(
            'post_type' => 'productos',
            'post_status' => 'publish',
        ...
        if ($productos->have_posts()) :
            ...
            while ($productos->have_posts()): $productos->the_post();
                    $item=wp_get_post_tags(get_the_Id());

                    foreach ($item as $key => $value) {
                            
                            if(!in_array( $value , $tags[$id] ) ) {
                                $tags[$id][$i]=$value;
                                $i++;
                             }
...
// Get Tags 
////////////////////////////////////////////////////////////////////
function retrive_tags($tags,$cat_id,$cat_link)
    {
    $tags_category='';
    if(!empty($tags[$cat_id])):
        foreach ($tags[$cat_id] as $tag => $value ) :
            $tags_category.='<li><a href="'.$cat_link.'?tag='.$value->name.'">'.$value->name.'</a></li>';                                                   
        endforeach;
    endif;
    return $tags_category;
}

function category_header($category_slug,$tag){
        $output=ucfirst($category_slug);
        if ($tag) { $output.=' - '.$tag; }
        return $output;    
}
